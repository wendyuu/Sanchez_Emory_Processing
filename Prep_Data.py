...
TunugzipCmd = Template('/tools/Slicer3/Slicer3-3.4.2-2010-01-06-linux-x86_64/bin/unu save -f nrrd -e gzip -i $infile -o $outfile')
TunujoinCmd = Template('/tools/Slicer3/Slicer3-3.4.2-2010-01-06-linux-x86_64/bin/unu join -a 3 -i $infile |/tools/Slicer3/Slicer3-3.4.2-2010-01-06-linux-x86_64/bin/unu save -f nrrd -e gzip -o $outfile')
...
for line in prefixlist:
   ...
   if(options.nodicom == False):
      zipname = os.path.join(ORIG_DIR,'orig',prefix+'_'+suffix)

      # unzip the data
      print 'Now Processing Subject ' + prefix + suffix
      print zipname
      if (os.path.exists(zipname) == False):
         print 'unzip ' + zipname + '.zip -d ' + zipname
         os.mkdir(zipname)
         os.system('unzip ' + zipname + '.zip -d ' + zipname)
   
      dicom_loc = os.path.join(zipname,suffix)
      for folder in os.listdir(dicom_loc):   
         # convert T1_06mm
         print folder
	   ...
	   os.system('DicomConvert ' + os.path.join(dicom_loc,folder) + ' ' + T16name +' -v')
           cmd = TunugzipCmd.substitute(infile=T16name,outfile=T16name)
           print cmd
           os.system(cmd)
	   #os.system('rm -rf ' + os.path.join(dicom_loc,folder))
           # convert T1_05mm
	    ...
	    os.system('DicomConvert ' + os.path.join(dicom_loc,folder) + ' ' + T2name +' -v')
	   ...
	   # os.system('rm -rf ' + os.path.join(dicom_loc,folder))
	    ...
	    os.system('DicomConvert ' + os.path.join(dicom_loc,folder) + ' ' + T15name +' -v')
            # os.system('rm -rf ' + os.path.join(dicom_loc,folder))
            ...
            dicomname = os.path.join(dicom_loc,folder)
   ...
   if(options.smrionly == False):
      ...
      for i in range(0,DWI_counter):
         ...
         raw_file_list = raw_file_list + ' ' + DWIname
      ...
      for i in range(0,B0_counter):
         ...
         raw_file_list = raw_file_list + ' ' + B0name
      ...
      zipCmd = 'zip '+zipname + ' -mu '+raw_file_list+' '+raw_file_list.replace('.nhdr','.raw.gz')
      print zipCmd
      os.system(zipCmd)
