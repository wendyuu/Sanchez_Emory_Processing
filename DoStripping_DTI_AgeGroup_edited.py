...
# SYSTEM VARIABLES
#/tools/Slicer3/Slicer3-3.4.1-2009-10-09-linux-x86_64/bin/unu
from string import Template
TaregCmd = Template('/tools/rview_linux64_2008/areg $tar $sou')
TAregCmd = Template('/tools/rview_linux64_2008/Areg $tar $sou')
TclampCmd = Template('/home/hartg/teem/teem-build/bin/unu 3op clamp $min $infile $max -o $infile')
TupsampleCmd = Template('/home/hartg/teem/teem-build/bin/unu resample -i $sou -o $out -s $dim -k hann:15')
...
TunusaveCmd = Template('/home/hartg/teem/teem-build/bin/unu save -f nrrd -i $infile -o $outfile')
...
unuhead = '/home/hartg/teem/teem-build/bin/unu head '
...
for line in prefixlist:
		...
		if(os.path.exists(BM_DTI)==False):
		   ...
		   if(os.path.exists(BM_DTI)==False):
			   ...
			   reorientCmd = TreorientCmd.substitute(infile = BM_DTI,outfile = BM_DTI);
